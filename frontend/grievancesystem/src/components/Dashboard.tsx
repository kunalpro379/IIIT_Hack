...
import { ChevronRight, Megaphone, Vote, Building, TrendingUp, Users, Clock, MessageSquare, HelpCircle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ language, theme, setShowForm }) => {
  ...
  const [visibleCards, setVisibleCards] = useState<number[]>([0, 1, 2]); // Show 3 cards at a time
  const [carouselPosition, setCarouselPosition] = useState(0);
  const [activeFeatures, setActiveFeatures] = useState<number[]>([0, 1, 2, 3]);
    ...
    <div className="space-y-16 -mt-8"> {/* Add negative margin to compensate for navbar padding */}
      ...
      >
        ...
        {!showLocalForm ? (
          ...
          <motion.div 
            ...
            className="relative z-20 container mx-auto px-4 h-full flex flex-col items-center justify-center text-center"
          >
            <div className="max-w-4xl mx-auto">
              ...
              >
                <div className="flex flex-col items-center gap-6">
                  <div 
                    className="my-2 mx-auto drop-shadow-2xl filter brightness-125" 
                  />
                  {/* Ashok Stambh */}
                  <img 
                ...
                <h1 className="text-5xl md:text-7xl font-bold mb-6 text-white mt-6">
                  UP-IGRS
                  <span className="block text-2xl md:text-3xl mt-2 text-[#FFA41B]">
              ...
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                className="px-8 my-5 py-4 bg-gradient-to-r from-[#19376D] to-[#0B2447] rounded-lg font-semibold text-white hover:from-[#FFA41B] hover:to-[#FF8C00] transition-all duration-300 shadow-lg"
              >
      ...
      {!showLocalForm && (
        <>
          ...
          >
            {[
              { title: "Total Grievances", value: "12,345", change: "+12%", icon: TrendingUp },
              { title: "Resolved Cases", value: "10,890", change: "+8%", icon: Clock },
              { title: "Average Resolution Time", value: "48h", change: "-25%", icon: Users }
            ].map((stat, i) => (
              <motion.div
                key={i}
          ...
          >
            {governmentAds.map((ad, index) => (
              <motion.div
                ...
                whileHover={{ scale: 1.02 }}
                className="group relative overflow-hidden rounded-xl bg-gradient-to-br from-[#0B2447]/90 to-black/90 border border-[#19376D]/30 hover:shadow-2xl hover:shadow-[#FFA41B]/20 transition-all duration-500"
              >
                {/* Glow effect */}
                <div className="absolute -inset-0.5 bg-gradient-to-r from-[#FFA41B]/20 to-[#FFD700]/20 rounded-xl blur opacity-0 group-hover:opacity-100 transition duration-500"></div>
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    >
                      <div className="p-2 rounded-lg bg-white/10 backdrop-blur-sm">
                        {React.createElement(ad.icon, { className: "w-5 h-5 text-purple-300" })}
                      </div>
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-purple-300 font-semibold">{ad.stats}</span>
                      <motion.button
                        whileHover={{ x: 5 }}
          ...
          >
            {ads.map((ad, index) => (
              <motion.div
                key={index}
                whileHover={{ scale: 1.02 }}
                className="group relative overflow-hidden rounded-xl bg-black/50 backdrop-blur-sm border border-white/10 hover:border-purple-500/50 transition-all duration-300"
              >
                <div className="aspect-video overflow-hidden">
                  <img 
                    src={ad.image} 
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="p-2 rounded-lg bg-purple-500/20 backdrop-blur-sm">
                      {React.createElement(ad.icon, { className: "w-5 h-5 text-purple-400" })}
                    </div>
